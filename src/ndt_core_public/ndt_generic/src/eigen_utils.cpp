...
double GetDistance(const Eigen::Affine3d &p1, const Eigen::Affine3d &p2,
  ...
  return diff.translation().norm() + alpha * k * euler.norm();
}
...
double SearchForClosestElement(const Eigen::Affine3d &pose,
  ...
  for (int i = 0; i < affine3d_vek.size(); i++) {
    double distance = GetDistance(pose, affine3d_vek[i], k, alpha);
...
Eigen::Affine3d getAffine3dMean(const std::vector<Eigen::Affine3d> &Ts) {
  ...
  Eigen::JacobiSVD<Eigen::Matrix3d> svd(sumRot, Eigen::ComputeFullU |
                                                    Eigen::ComputeFullV);
...
pos<<x,y,z;
rot<<r,p,t;
return vectorsToAffine3d(pos,rot);
}
...
Eigen::Affine3d getWeightedPose(const Eigen::Affine3d &pose1,
  ...
  Eigen::VectorXd out =
      (cov1_inv + cov2_inv).inverse() * (cov1_inv * p1 + cov2_inv * p2);
...
Eigen::Affine3d Extrapolate(const Eigen::Affine3d &T1,
                            const Eigen::Affine3d &T2, double stepsize) {
  Eigen::Affine3d result;
  result = T2 * (T1.inverse() * T2) *
           Eigen::Scaling(
               stepsize); // factor=1 corresponds to an step equal to a frame
}
Eigen::Affine3d Interpolate(const Eigen::Affine3d &T1,
                            const Eigen::Affine3d &T2, double factor) {
...
Eigen::Affine3f Interpolate(const Eigen::Affine3f &T1,
                            const Eigen::Affine3f &T2, double factor) {
  // std::cout<<"factor="<<factor<<std::endl;
  Eigen::Affine3f pose;
...
Eigen::Matrix3d Cov6dTo3d(const Eigen::Matrix<double, 6, 6> &Cov){
  ...
  cov_2d<<Cov(0,0), Cov(0,1), Cov(0,5),
