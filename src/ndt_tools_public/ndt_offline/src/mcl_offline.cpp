...
bool ReadAllParameters(po::options_description &desc,int &argc, char ***argv){
  ...
  desc.add_options()
      ...
      ("min_dist", po::value<double>(&min_keyframe_dist)->default_value(0.2), "minimum distance traveled before adding cloud")
      ("min_rot_in_deg", po::value<double>(&min_keyframe_rot_deg)->default_value(5), "minimum rotation before adding cloud")
      ...
      ("velodyne_packets_topic", po::value<std::string>(&lidar_topic)->default_value(std::string("/velodyne_packets")), "velodyne packets topic used")
...
int main(int argc, char **argv){
  ...
  base_name += motion_params.getDescString() + std::string("_res") + toString(resolution) + std::string("_SC") + toString(do_soft_constraints) + std::string("_mindist") + toString(min_keyframe_dist) + std::string("_sensorcutoff") + toString(max_range) + std::string("_stepcontrol") + toString(step_control) + std::string("_neighbours") + toString(nb_neighbours) + std::string("_rlf") + toString(resolution_local_factor);
  ...
  for(int i=0; i<ros_bag_paths.size(); i++) {
    ...
    cout<<velodyne_config_file<<","<<bagfilename<<","<<lidar_topic<<","<<velodyne_frame_id<<","<<tf_world_frame<<","<<tf_topic<<endl;
    ...
    while(vreader.readMultipleMeasurements(nb_scan_msgs,cloud_nofilter,tf_scan_source,tf_gt_base,base_link_id)){
     ...
     //   graph_map->SwitchToClosestMapNode(fuser_pose,unit_covar,T,std::numeric_limits<double>::max());
        ...
        //eval_files.Write( vreader.getTimeStampOfLastSensorMsg(),Tgt_base*sensor_offset,Todom_base*sensor_offset,fuser_pose*sensor_offset);
        eval_files.Write( vreader.getTimeStampOfLastSensorMsg(),Tgt_base,Todom_base,fuser_pose,fuser_pose*sensor_offset);
