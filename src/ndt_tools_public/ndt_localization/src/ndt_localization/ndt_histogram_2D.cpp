...
perception_oru::histogram_cell::histogram_cell(perception_oru::NDTMap *map_, double te_, int ns_, int np_, std::vector<double> begins_, std::vector<double> ends_, double cs_, Eigen::Vector2d origin_,  int os_, double opening_)
  : histogram_definition(te_, ns_, np_, begins_, ends_, cs_, origin_, os_, opening_), contributing_cells(0){
  // ROS_INFO_STREAM(__FILE__<<","<<__LINE__);
  histograms.resize(os);
  for(int i = 0; i < os; ++i)
...
perception_oru::histogram_map::histogram_map(perception_oru::NDTMap *map_, double te_, int ns_, int np_, std::vector<double> begins_, std::vector<double> ends_, double range_, int os_, double opening_){
