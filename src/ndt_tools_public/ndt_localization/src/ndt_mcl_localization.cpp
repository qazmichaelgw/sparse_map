  ...
  class particle_filter_wrap {
    ...
    void processFrame(pcl::PointCloud<pcl::PointXYZ> &cloud, ros::Time ts){
      ...
      //ROS_INFO_STREAM("mean pose"<<mean_pose);
  ...
  public:
    particle_filter_wrap(ros::NodeHandle param){
      ...
      param.param<std::string>("init_type", initType, "normal");


      param.param<double>("roundess", te, 0.9);
      param.param<double>("range", range, 80);
      param.param<int>("number_spherical_classes", ns, 2);
      param.param<int>("number_planar_classes", np, 36);
      param.param<double>("opening", opening, 2 * M_PI);
      param.param<int>("number_of_orientations", os, 36);


      XmlRpc::XmlRpcValue be, en;
      param.param("begins", be, be);
      for(int i =0; i < be.size(); i++){
        begins.push_back(static_cast<double>(be[i]));
      }

      param.param("ends", en, en);
      for(int i =0; i < en.size(); i++){
        ends.push_back(static_cast<double>(en[i]));
      }
      ...
      SetInitialization();
        if(ndt)
          MCL->SetupNDTH(te, ns, np, begins, ends, range, os, opening);
      if(beVelodyne)
